...
public class DispStockListZaiko {

	// index値の項目が「数字」であることを示す配列
	private int[] numFlag = new int[14];
	 ...
	 * @return モデルの配列を返します。
	 */
	public StockListZaikoMdl[] getTmpItem(StockListZaikoMdl[] item,
		...
		for (int i = 0; i < item.length; i++) {
			...
			case 6:
				...
				break; // 単価率：実数
			case 7:
				strArr[i] = item[i].getStrKurikosi();
				break; // 繰越量：実数
			case 8:
				strArr[i] = item[i].getStrNyuko();
				break; // 入庫量：実数
			case 9:
				strArr[i] = item[i].getStrHarai();
				break; // 払出量：実数
			case 10:
				strArr[i] = item[i].getStrHenpin();
				break; // 返品量：実数
			case 11:
				strArr[i] = item[i].getStrTyousei();
				break; // 調整量：実数
			case 12:
		...
		StockListZaikoMdl[] tmpItem = new StockListZaikoMdl[item.length];
		// for(int i=0; i<numFlag.length; i++){
		// if(numFlag[i]){
	 ...
	 */
	public Vector getVdata(StockListZaikoMdl[] tmpItem) {
		...
		for (int i = 0; i < tmpItem.length; i++) {
			...
			vcdata.addElement(tmpItem[i].getStrBase()); // 薬価基準：実数
			vcdata.addElement(tmpItem[i].getStrStock()); // 在庫単価：実数
			vcdata.addElement(tmpItem[i].getStrRate()); // 単価率：実数
			vcdata.addElement(tmpItem[i].getStrKurikosi()); // 繰越量：実数
			vcdata.addElement(tmpItem[i].getStrNyuko()); // 入庫量：実数
			vcdata.addElement(tmpItem[i].getStrHarai()); // 払出量：実数
			vcdata.addElement(tmpItem[i].getStrHenpin()); // 返品量：実数
			vcdata.addElement(tmpItem[i].getStrTyousei()); // 調整量：実数
