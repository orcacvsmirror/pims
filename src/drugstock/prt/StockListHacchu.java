...
import drugstock.biz.BizStockListHacchu;
...
public class StockListHacchu extends StockList {

	final int PAGE_MAX_ROW = 55; // １ページの明細数
	final boolean myDebug = false;
	public static String TITLE_HACCHU = "＊＊発注点未満 薬剤リスト＊＊";

	public StockListHacchu() {
		super(PRT_DEAD_STOCK, TITLE_HACCHU);
	}
	...
	public void run() {
		...
		try {
			...
			// if(dead_stock_month == null) dead_stock_month = "0";
			...
			BizStockListHacchu biz = new BizStockListHacchu();
			item = biz.getListData(strNowDate, iContArr, itemKindArr);
			waitData.destroy();

			// 画面表示
			if (bTable) {
				String[] columnHead = { "業者名", "区分", "品番", "薬剤名", "発注点", "薬剤量",
				        "在庫金額" };
				...
				DispTableDlg dlg = new DispTableDlg(ht, columnHead,
				        columnWidth, PRT_HACCHU);
				dlg.setItemHacchu(item);
			...
			if (printFlg.equals("csv")) {
				...
				if (csvName == null)
					csvName = "/tmp/";
				csvName += "stocklistHacchu" + strNowDate + ".csv";
				...
				for (int i = 0; i < item.length; i++) {
					...
					bw = getCSVEl(bw, item[i].getBytesHacchuP(), "camma");
			...
			if (printFlg.equals("print")) {
				...
				for (int i = 0; i < iContArr.length; i++) {
					...
					while (bLoop) {
						...
						// 該当業者の抽出
						StockListHacchuMdl itemTmp[] = new StockListHacchuMdl[count];
						int k = 0;
						for (int j = 0; j < item.length; j++) {
							if (item[j].getStrCont().equals(contKind)) {
								itemTmp[k] = item[j];
						...
						// System.out.println( itemTmp.length );
						...
						for (int j = ps; j < pe; j++) {
							...
							if (j < itemTmp.length) {
								...
								// ;40
								bw.write(itemTmp[j].getBytesHacchuP()); // 発注点
								// ;12
