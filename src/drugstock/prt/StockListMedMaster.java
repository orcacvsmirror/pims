...
import drugstock.biz.BizStockListMedMaster;
...
public class StockListMedMaster extends StockList {

	final int PAGE_MAX_ROW = 55; // １ページの明細数
	final boolean myDebug = false;
	public static String TITLE_MED_MASTER = "＊＊＊薬剤マスタ一覧表＊＊＊";

	public StockListMedMaster() {
		// super(PRT_MED_MASTER,"＊＊＊薬剤マスタ一覧表＊＊＊");
		super(PRT_MED_MASTER, TITLE_MED_MASTER);
	}
	...
	public void run() {
		...
		// 薬剤区分毎在庫金額集計表
		try {
			StockListMedMasterMdl item[] = null;
			...
			BizStockListMedMaster biz = new BizStockListMedMaster();
			item = biz.getListData(strNowDate, iContArr, itemKindArr);
			waitData.destroy();

			// 画面表示
			if (bTable) {
				String[] columnHead = { "業者名", "区分", "品番", "薬剤名", "単位", "薬価" };
				int[] columnWidth = { 100, 50, 80, 300, 80, 80 };
				Hashtable ht = new Hashtable();
				ht.put("nengetu", strNowDate);

				DispTableDlg dlg = new DispTableDlg(ht, columnHead,
				        columnWidth, PRT_MED_MASTER);
				dlg.setItemMedMaster(item);
			...
			if (printFlg.equals("csv")) {
				...
				if (csvName == null)
					csvName = "/tmp/";
				csvName += "stocklistMedMaster" + strNowDate + ".csv";
			...
			if (printFlg.equals("print")) {
				...
				int ps, pe, pcnt, kcnt;
				String fileName = dataDir + "stocklist_common.dat";
				String formName = formDir + "stocklist_common.red";
				String pageNo = null;
				boolean bLoop = false;
				...
				for (int i = 0; i < iContArr.length; i++) {
					...
					while (bLoop) {
						...
						bw.write(com.getByteData(TITLE_MED_MASTER, 40));

						// 凡例の出力
						bw.write(com.getByteData("区分", 8));
						bw.write(com.getByteData("品番", 10));
						bw.write(com.getByteData("薬剤名", 50));
						bw.write(com.getByteData("単位", 8));
						bw.write(com.getByteData("薬価", 10));
						...
						// 該当薬剤の抽出
						StockListMedMasterMdl itemTmp[] = new StockListMedMasterMdl[count];
						int k = 0;
						for (int j = 0; j < item.length; j++) {
							if (item[j].getStrCont().equals(contKind)) {
								itemTmp[k] = item[j];
						...
						for (int j = ps; j < pe; j++) {
							...
							if (j < itemTmp.length) {
								...
								// ;50
								bw.write(itemTmp[j].getBytesUnit()); // 単位 ;8
								bw.write(itemTmp[j].getBytesStockPrice()); // 薬価
								// ;10
							} else {
								bw.write(com.getByteData("", 86)); // 空白
							}
						}
						bw.close();
						kcnt++;
